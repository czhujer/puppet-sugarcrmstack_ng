...
class sugarcrmstack_ng::mysql_server (
  ...
  if ($mysql_server_enable){
    ...
    if ($mysql_server_use_pxc == true and ($sugar_version == '7.9' or $sugar_version == '8.0' or $sugar_version == '9.0') and $::operatingsystemmajrelease in ['7'] ){
    ...
    if ($sugar_version == '7.5' or $sugar_version == '7.9' or $sugar_version == '8.0' or $sugar_version == '9.0'){
      ...
      fail("${class_name}: This class is compatible only with sugar_version 7.5,7.9,8.0 or 9.0 (not ${sugar_version})")
    }
  }
}
